...
from quadrotor import QuadrotorModel, rotate_quat,QuadrotorModel_Traj,QuadrotorSimpleModel
...
class TrackerMPC():
    def __init__(self, quad:QuadrotorSimpleModel):
        ...
        self._Trj_p = ca.SX.sym("Trj_p", 3, self._trj_Point)
        self._Trj_yaw = ca.SX.sym("Trj_yaw", self._trj_Point)
        ...
        for i in range(self._trj_Point):
    ...
    def solve(self, xinit, Trjp, Trjyaw):
        p = np.zeros(self._X_dim + 3*self._trj_Point + 1*self._trj_Point)
        #load param
        p[:self._X_dim] = xinit
        p[self._X_dim:self._X_dim+3*self._trj_Point] = Trjp
        p[self._X_dim+3*self._trj_Point:self._X_dim+4*self._trj_Point] = Trjyaw
...
if __name__ == "__main__":
    ...
    # system('gcc -fPIC -shared -O3 ' + "tracker_pos.c" + ' -o ' + './robotracker.so')

...
namespace ego_planner
{
  ...
  bool EGOPlannerManager::reboundReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
                                        ...
                                        Eigen::Vector3d local_target_vel, Eigen::Vector3d wps_1,Eigen::Vector3d wps_2,Eigen::Vector3d wpv_1,int wp_id_,bool flag_polyInit, bool flag_randomPolyTraj)   // flag_polyInit true; flag_randomPolyTraj
  {
    static int count = 0;
    ...
    bspline_optimizer_->setLocalTargetPt(local_target_pt,wp_id_);
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        PolynomialTraj gl_traj,gl_traj1_,gl_traj2_;
        gl_traj.reset();
        double dist = (start_pt - local_target_pt).norm();
        double time = pow(pp_.max_vel_, 2) / pp_.max_acc_ > dist ? sqrt(dist / pp_.max_acc_) : (dist - pow(pp_.max_vel_, 2) / pp_.max_acc_) / pp_.max_vel_ + 2 * pp_.max_vel_ / pp_.max_acc_;

        if((((wps_1[0] - local_target_pt[0])*(wps_1[0] - start_pt[0]) < 0 && abs(wpv_1[0]) > 0) || 
        ((wps_1[1] - local_target_pt[1])*(wps_1[1] - start_pt[1]) < 0 && abs(wpv_1[1]) > 0))&&(start_pt - wps_1).norm() > 1.0&&(local_target_pt - wps_1).norm() > 1.0 )
        {
          gl_traj1_ = PolynomialTraj::one_segment_traj_gen(start_pt, start_vel, start_acc, wps_1, wpv_1, Eigen::Vector3d::Zero(), time/2);
          gl_traj2_ = PolynomialTraj::one_segment_traj_gen(wps_1, wpv_1, Eigen::Vector3d::Zero(), local_target_pt, local_target_vel, Eigen::Vector3d::Zero(), time/2);
          gl_traj.addSegment(gl_traj1_.getCoef(0)[0], gl_traj1_.getCoef(1)[0], gl_traj1_.getCoef(2)[0], gl_traj1_.getTimes()[0]);
          gl_traj.addSegment(gl_traj2_.getCoef(0)[0], gl_traj2_.getCoef(1)[0], gl_traj2_.getCoef(2)[0], gl_traj2_.getTimes()[0]);
          ROS_INFO("PolynomialTraj::addSegment");          
        }else{
          gl_traj = PolynomialTraj::one_segment_traj_gen(start_pt, start_vel, start_acc, local_target_pt, local_target_vel, Eigen::Vector3d::Zero(), time);
          ROS_INFO("PolynomialTraj::one_segment_traj_gen");
        }
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        if (poly_time > ts)
        {
          PolynomialTraj gl_traj,gl_traj_1,gl_traj_2;
          gl_traj.reset();
          if((((wps_1[0] - local_target_pt[0])*(wps_1[0] - local_data_.position_traj_.evaluateDeBoorT(t)[0]) < 0 && abs(wpv_1[0]) > 0) || 
          ((wps_1[1] - local_target_pt[1])*(wps_1[1] - local_data_.position_traj_.evaluateDeBoorT(t)[1]) < 0 && abs(wpv_1[1]) > 0))&&(local_data_.position_traj_.evaluateDeBoorT(t) - wps_1).norm() > 1.0&&(local_target_pt - wps_1).norm() > 1.0 )
          {
            gl_traj_1 = PolynomialTraj::one_segment_traj_gen(local_data_.position_traj_.evaluateDeBoorT(t),
                                                                          local_data_.velocity_traj_.evaluateDeBoorT(t),
                                                                          local_data_.acceleration_traj_.evaluateDeBoorT(t),
                                                            wps_1, wpv_1, Eigen::Vector3d::Zero(), poly_time/2);
            gl_traj_2 = PolynomialTraj::one_segment_traj_gen(wps_1, wpv_1, Eigen::Vector3d::Zero(),
                                                                          local_target_pt, local_target_vel, Eigen::Vector3d::Zero(), poly_time/2);
            gl_traj.addSegment(gl_traj_1.getCoef(0)[0], gl_traj_1.getCoef(1)[0], gl_traj_1.getCoef(2)[0], gl_traj_1.getTimes()[0]);
            gl_traj.addSegment(gl_traj_2.getCoef(0)[0], gl_traj_2.getCoef(1)[0], gl_traj_2.getCoef(2)[0], gl_traj_2.getTimes()[0]);
            ROS_INFO("PolynomialTraj::addSegment");          
          }else{
            gl_traj = PolynomialTraj::one_segment_traj_gen(local_data_.position_traj_.evaluateDeBoorT(t),
                                                                          local_data_.velocity_traj_.evaluateDeBoorT(t),
                                                                          local_data_.acceleration_traj_.evaluateDeBoorT(t),
        ...
        // cout << "point_set.size()" << point_set.size()<<endl;
        // cout << "point_pplan()" << pp_.planning_horizen_ / pp_.ctrl_pt_dist * 3 <<endl;
    ...
    {
      // cout << "enter" << endl;
      std::vector<ControlPoints> trajs = bspline_optimizer_->distinctiveTrajs(segments,wp_id_);
      // cout << "\033[1;33m"
  ...
  bool EGOPlannerManager::planGlobalTraj(const Eigen::Vector3d &start_pos, const Eigen::Vector3d &start_vel, const Eigen::Vector3d &start_acc,
                                         const Eigen::Vector3d &mid_pos, const Eigen::Vector3d &mid_vel, const Eigen::Vector3d &mid_acc,
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    PolynomialTraj gl_traj_1,gl_traj_2,gl_traj;
    gl_traj.reset();
    // if (pos.cols() >= 3)
    //   gl_traj = PolynomialTraj::minSnapTraj(pos, start_vel, end_vel, start_acc, end_acc, time);
    // else if (pos.cols() == 2)
    ...
    // else
    //   return false;
    gl_traj_1 = PolynomialTraj::one_segment_traj_gen(start_pos, start_vel, start_acc, mid_pos, mid_vel, mid_acc, time(0));
    gl_traj_2 = PolynomialTraj::one_segment_traj_gen(mid_pos, mid_vel, mid_acc, end_pos, end_vel, end_acc, time(1));
    gl_traj.addSegment(gl_traj_1.getCoef(0)[0], gl_traj_1.getCoef(1)[0], gl_traj_1.getCoef(2)[0], gl_traj_1.getTimes()[0]);
    gl_traj.addSegment(gl_traj_2.getCoef(0)[0], gl_traj_2.getCoef(1)[0], gl_traj_2.getCoef(2)[0], gl_traj_2.getTimes()[0]);

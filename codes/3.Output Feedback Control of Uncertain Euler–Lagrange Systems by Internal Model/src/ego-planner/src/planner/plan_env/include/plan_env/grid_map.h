...
public:
  ...
  inline int getInflateOccupancy(Eigen::Vector3d pos,int wp_id);
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos,int wp_id) {
  if (!isInMap(pos)) return -1;
  if (wp_id == 10){
    Eigen::Vector3i id;
    posToIndex(pos, id);
    // std::cout << "wp_id == 1" << std::endl;
    return int(md_.occupancy_buffer_inflate_[toAddress(id)]);    
  }else{
    // std::cout << "wp_id  " << std::endl;

...
void GlobalOptimization::inputGPS_ZERO(double t, double latitude, double longitude, double altitude, double posAccuracy)
{
	double xyz[3];
	// GPS2XYZ(latitude, longitude, altitude, xyz);
    xyz[0] = latitude;
    xyz[1] = -longitude;
    xyz[2] = -altitude;
	// vector<double> tmp{xyz[0], xyz[1], xyz[2],  1.0};
    vector<double> tmp{xyz[1], xyz[0], -xyz[2],  5.0};
    //printf("new gps: t: %f x: %f y: %f z:%f \n", t, tmp[0], tmp[1], tmp[2]);
	GPSPositionMap[t] = tmp;
    newGPS = true;

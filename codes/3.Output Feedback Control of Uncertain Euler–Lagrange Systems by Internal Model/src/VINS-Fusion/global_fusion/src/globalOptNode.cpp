...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    while(!gpsQueue.empty())
    {
        ...
        bool STA_check = (GPS_msg->header.stamp.toSec() <= 0);
        if (STA_check)
        {
            ...
            if(GPS_msg->status.status == 7)
            {
                globalEstimator.inputGPS_ZERO(t, latitude, longitude, altitude, pos_accuracy);
            }
            else
            {   
            ...
            // gpsQueue.pop();
            while (!gpsQueue.empty()) gpsQueue.pop();
            break;
        }
        double gps_t = GPS_msg->header.stamp.toSec();
        // printf("vio t: %f, gps t: %f \n", t, gps_t);

...
class AITEPoseV2:
    def __init__(
        ...
        self.smoother = sc1v2(window_size=window_size_s).to(self.device)
        ...
        self.pred_seq = torch.zeros(1, self.window_size, 3).to(self.device)
        self.window_seq = torch.zeros(1, self.window_size - 1, 3).to(
            self.device
        )  # TOO 这里只限定17关节

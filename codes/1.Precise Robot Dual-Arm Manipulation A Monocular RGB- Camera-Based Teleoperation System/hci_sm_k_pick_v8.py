...
FilePath: hci_sm_k_pick_v8.py
Description  : 3DHPE+sw滤波+手势识别
version      : 
-更新映射 左右圆形变动yam 以root为原点
-pick 引入手掌识别
-v7 基于肘到肩2D距离的自适应滤波算法,抓取判别优化
-v8 基于肘、肩2D坐标的自适应滤波算法,效果也不错
notion       : 
'''
...
class HPE_3D:
    def __init__(
        ...
        self.scorelist = []
        self.bbox_list = []
        self.filter = AdaptiveFilter_ref(s=16,n=3,d=0,r=0.3,n_ref=2,d_ref=2,max_r=0.3,min_r=0,smoothing_factor=0.6,sensitivity=1/3)
        self.filter2 = AdaptiveFilter_ref(s=16,n=3,d=0,r=0.3,n_ref=2,d_ref=2,max_r=0.3,min_r=0,smoothing_factor=0.6,sensitivity=1/4)
        ...
        self.gripper=0
        self.gripper2=0
        self.hand_right_manager = HandStateManager(confidence_threshold=0.8,frame_threshold=5)
        self.hand_left_manager = HandStateManager(confidence_threshold=0.8,frame_threshold=5)
        self.state1= []
        # self.state2 =[] #tem
        ...
        while True:
            ...
            if self.detector == 'anchor':
                ...
                if self.origin_3d is None:
                    ...
                    self.origin_o = coords_3d[0] # 为了给后面ref量做准备
                ...
                if not self.filte_start:
                    ...
                    coord_ref = (coords_3d[14:16,:2]-self.origin_o[:2]).cpu().numpy()
                    coord_ref2 = (coords_3d[11:13,:2]-self.origin_o[:2]).cpu().numpy()

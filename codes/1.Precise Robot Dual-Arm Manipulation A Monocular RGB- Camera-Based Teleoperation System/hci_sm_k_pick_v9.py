...
from utils.hand_open import calculate_hand_openness2,HandStateManager,are_vectors_collinear,length_rate
...
class HPE_3D:
    def __init__(
        ...
        self.gripper=0
        self.gripper2=0
        self.hand_right_manager = HandStateManager(confidence_threshold=0.83,frame_threshold=2)
        self.hand_left_manager = HandStateManager(confidence_threshold=0.83,frame_threshold=2)
        self.state1= []
        # self.state2 =[] #tem
        self.filte_start = False
        self.coords_handv = None
        self.coords_handv2 = None
        self.coords_handv_last = None
        self.coords_handv2_last = None
        self.is_gripper = 0
        self.is_gripper2 = 0
    ...
    def infer(self, function=None):
        ...
        while True:
            ...
            if self.detector == 'anchor':
                ...
                coords_3d_filt = (coords_3d_filt - self.origin_3d).cpu().numpy()
                coords_3d_filt2 = (coords_3d_filt2 - self.origin_3d2).cpu().numpy()
                ...
                if i % 1 == 0:
                    ...
                    mp_image1 = mp.Image(image_format=mp.ImageFormat.SRGB, data=hand_img)
                    ...
                    self.right_hand_recognizer.detect_async(mp_image1, 2*i)  
                    self.left_hand_recognizer.detect_async(mp_image, 2*i+1)
                
                # tag 添加抓取锁定
                if self.hand_img:
                    if self.hand_right_manager.switch :
                        self.is_gripper = 10
                    if self.hand_left_manager.switch:    
                        self.is_gripper = 10
                
                if not self.filte_start:
                    self.filter.init_history(coords_3d_filt)
                    self.filter2.init_history(coords_3d_filt2)
                    self.filte_start = True
                    self.hand_point = coords_3d_filt
                    self.hand_point2 = coords_3d_filt2
                else:
                    if self.is_gripper==0:
                        coord_ref = (coords_3d[15,:2]-coords_3d[14,:2]).cpu().numpy()
                        self.hand_point,factor = self.filter.update(coords_3d_filt,coord_ref)
                    else:
                        self.is_gripper = self.is_gripper - 1
                    if self.is_gripper2==0:
                        coord_ref2 = (coords_3d[12,:2]-coords_3d[11,:2]).cpu().numpy()
                        self.hand_point2,factor2 = self.filter2.update(coords_3d_filt2,coord_ref2)
                    else:
                        self.is_gripper2 = self.is_gripper2 - 1
                    self.templist_kf.append(np.append(self.hand_point,int(self.is_gripper>0)))  # 保存滤波后的数据
                    self.templist_kf2.append(np.append(self.hand_point2,int(self.is_gripper2>0)))  # 保存滤波后的数据
                # 发布交互数据
                out_hand_pose = [
                    self.hand_point[0],
                    self.hand_point[1],

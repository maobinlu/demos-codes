...
FilePath: hci_sm_k_pick_v6.py
Description  : 3DHPE+sw滤波+手势识别
version      : 基于肘和肩2D坐标的 自适应滤波算法
notion       : 更新映射，左右圆形变动yam，以root为原点,引入手掌识别
'''
...
class HPE_3D:
    def __init__(
        ...
        self.scorelist = []
        self.bbox_list = []
        self.filter = AdaptiveFilter_ref(s=16,n=3,d=0,r=0.3,n_ref=3,d_ref=2,max_r=0.4,min_r=0,smoothing_factor=0.8)
        self.filter2 = AdaptiveFilter_ref(s=16,n=3,d=0,r=0.3,n_ref=3,d_ref=2,max_r=0.4,min_r=0,smoothing_factor=0.8)
    ...
    def right_hand_callback(self, result, output_image: mp.Image, timestamp_ms: int):
        ...
        self.state1.append(np.array(finger_bend_angles)) #temp 检测手指弯曲角度
    ...
    def infer(self, function=None):
        ...
        while True:
            ...
            if i == 0:
                pre_w = bbox[2]
                pre_h = bbox[3]
                # self.box_filter.init_history(bbox) #tag去除了bbox滑动平均滤波
            ...
            if self.detector == 'anchor':
                ...
                if not self.filte_start:
                    ...
                    coord_ref = coords_3d[14:17,:2].cpu().numpy()
                    coord_ref2 = coords_3d[11:14,:2].cpu().numpy()

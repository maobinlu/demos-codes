...
dict_path_50 = "hci_0v1.pth"
dict_path = "hci_0v1_152.pth"
joint_num = 22
video_name = 'ROKI'
video_file = 'video'
...
# fc = [451.477162,0.000000 ,272.213870,
# 0.000000, 453.875205, 294.493310,
# 0.000000, 0.000000, 1.000000]
f = [451.477162, 453.875205]
c = [272.213870, 294.493310]

if __name__ == "__main__":

    # model = Pose_inference(dict_path=dict_path_50, resnet=50, input_h=256, input_w=256)
    model = Pose_inference(dict_path=dict_path, resnet=152, joint_num=joint_num, input_h=288, input_w=384)
    model_det = YOLO("yolov8n.pt")
    # model_det = YOLO("yolov8n.engine", task="detect")
    # model_det = YOLO("yolov8n.onnx", task="detect")
    print("start predict")
    # fourcc = cv2.VideoWriter_fourcc(*"H264")
    # capture = cv2.VideoCapture(video_path)
    capture = cv2.VideoCapture(0)  # 使用相机
    ...
    scorelist = []
    bbox_list = []
    box_filter = Filter(s=32, n=4, r=1)
    pose_filter = Filter(s=16, n=joint_num, d=3, r=1)
    while True:
        t1 = time.time()
        ...
        results = model_det.predict(frame, save=False, imgsz=320, classes=0, conf=0.3)
        ...
        if i == 0:
            pre_w = bbox[2]
            pre_h = bbox[3]
            box_filter.init_history(bbox)
        bbox = box_filter.update(bbox)
        # bbox, pre_w, pre_h = bbox_transform(bbox.clone(), 1, pre_w, pre_h, width, height)
        bbox, pre_w, pre_h = bbox_transform(bbox.clone(), 288/384, pre_w, pre_h, width, height)
        coords, scores = model.predict(frame_p, bbox)
        if (coords[7, 1]-coords[10, 1]) > (bbox[3]-bbox[1])*0.4:
            scores[0, :7, :] = 0
        # pdb.set_trace()
        if i == 0:
            pose_filter.init_history(coords)
        coords = pose_filter.update(coords)
        coords_3d = coords.clone()
        coords_3d[:, 2] = coords_3d[:, 2] * 2000
        coords_3d[:, 0] = coords[:, 0] * (2000 / (bbox[2] - bbox[0]))
        coords_3d[:, 1] = coords[:, 1] * (2000 / (bbox[3] - bbox[1]))
        coords_3d = coords_3d - coords_3d[0]
        templist.append(coords_3d)
        scorelist.append(scores)
        bbox_list.append(bbox)
        frame_show = model.vis_keypoints(frame, coords.cpu(), scores.cpu(), kp_thresh=0.2)
        t = time.time() - t1
        fps = 1 / t

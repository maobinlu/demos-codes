...
class HPE_3D:
    def __init__(self, dict_path, smoother_weight,filter_weight, joint_num, window_size_s,window_size_f ,f, c, resnet):
        ...
        self.pose_pub2 = rospy.Publisher("/piper_left/pos_cmd", PosCmd, queue_size=1)
    ...
    def infer(self, function=None):
        ...
        while True:
            ...
            if self.detector == 'anchor':
                ...
                out_hand_pose2 = [self.hand_point2[0], self.hand_point2[1], self.hand_point2[2]]
                pub_pose2 = PosCmd()
                ...
                # spatial_mapping
                pub_pose.x = (pub_pose.x/1000-0.2)*4/3 +0.325
                pub_pose.y = (pub_pose.y/1000+0.05)*40/15
                pub_pose.z = (pub_pose.z/1000-0.05)*3/2 +0.41
                ...
                # pub_pose.data = out_hand_pose
                # spatial_mapping
                pub_pose2.x = (pub_pose2.x/1000-0.2)*4/3 +0.325
                pub_pose2.y = (pub_pose2.y/1000-0.05)*40/15
                pub_pose2.z = (pub_pose2.z/1000-0.1)*2 +0.41
                
                # 侧映射
                # pub_pose2.x = -out_hand_pose2[1]
                # pub_pose2.y = out_hand_pose2[0]
                # pub_pose2.z = out_hand_pose2[2]
                # pub_pose2.roll = 0
                # pub_pose2.pitch = 1.57
                # pub_pose2.yaw = 0
                # pub_pose2.x = pub_pose2.x/1000*1/2+0.25
                # pub_pose2.y = pub_pose2.y/1000 - 0.25
                # pub_pose.z = pub_pose.z/1000*5/8 +0.45 
                
                #抓取映射
                # pub_pose2.z = pub_pose2.z/1000 +0.4
                if pub_pose2.z < 0.12:
                    pub_pose2.z = 0.12
                if pub_pose2.z < 0.4:
                    pub_pose2.pitch=1.57+(0.4-pub_pose2.z)*0.83/0.3
                    # print('pitch',pub_pose.pitch)
                # print('pub_pose2.z',pub_pose2.z)
                if self.pick:
                    pub_pose2.gripper = 0.01
                else:
                ...
                if i % 10 == 0:
                    ...
                    x1, y1, x2, y2 = int(coords_2d_hand[0]-50), int(coords_2d_hand[1]-50), int(coords_2d_hand[0]+50), int(coords_2d_hand[1]+50)

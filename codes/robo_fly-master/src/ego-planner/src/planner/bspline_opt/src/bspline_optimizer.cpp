...
namespace ego_planner
{
  ...
  std::vector<ControlPoints> BsplineOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments,int wp_id_)
  {
    occ_wp_id_ = wp_id_;
    ...
    for (int i = 0; i < seg_upbound; i++)
    {
      ...
      if (RichInfoSegs[i].first.size > 1)
      {
        ...
        for (int j = 0; j < RichInfoSegs[i].first.size - 1; j++)
        {
          ...
          for (double a = 1; a > 0; a -= step_size)
          {
            ...
            if (grid_map_->getInflateOccupancy(pt,occ_wp_id_))
            {
              occ_start_id = j;
              occ_start_pt = pt;
      ...
      exit_multi_loop2:;
        ...
        for (int j = occ_start_id; j <= occ_end_id; j++)
        {
          ...
          if (grid_map_->getInflateOccupancy(base_pt_reverse,occ_wp_id_)) // Search outward.
          {
            ...
            for (; l <= l_upbound; l += RESOLUTION)
            {
              ...
              if (!grid_map_->getInflateOccupancy(base_pt_temp,occ_wp_id_))
              {
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      {
        occ = grid_map_->getInflateOccupancy(a * init_points.col(i - 1) + (1 - a) * init_points.col(i),occ_wp_id_);
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      if (a_star_->AstarSearch(0.1, in, out,occ_wp_id_))
      {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            ...
            {
              occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * init_points.col(j),occ_wp_id_);
  ...
  bool BsplineOptimizer::check_collision_and_rebound()
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      bool occ = grid_map_->getInflateOccupancy(cps_.points.col(i),occ_wp_id_);
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          occ = grid_map_->getInflateOccupancy(cps_.points.col(j),occ_wp_id_);
          if (!occ)
          {
            in_id = j;
            break;
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          if (got_intersection_id >= 0)
          {
            ...
            if (length > 1e-5)
            {
              ...
              for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
              {
                bool occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j),occ_wp_id_);
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        {
          flag_occ = grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t),occ_wp_id_);
          if (flag_occ)
          {
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      {
        if (grid_map_->getInflateOccupancy(traj.evaluateDeBoorT(t), occ_wp_id_))
        {
